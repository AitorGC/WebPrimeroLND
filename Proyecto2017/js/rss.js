...
function SetRSSTemplates()
{
	if (rssBody)
	{
		...
		{
			var output = (document.all) ? Replace(rssBody.innerHTML,"(:enlace:)",getNode('link',i))
									   : Replace(rssBody.innerHTML,"%28:enlace:%29",getNode('link',i));
	...
	if (rssTitle)
	{
		...
		output = (document.all) ? Replace(output,"(:enlace:)",getNode('link'))
